...
exports.up = async function (knex) {
    // 0) Drop existing tables (child → parent to avoid FK errors)
    ...
    await knex.schema.createTable('resorts', function (table) {
      ...
      table.integer('total_acres').notNullable();
      table.string('canyon_name', 50).notNullable();
      table.string('website', 200).notNullable();
      table.string('ski_patrol_phone', 12).notNullable();
      table.boolean('has_night_skiing').defaultTo(false).notNullable();
    ...
    await knex.schema.createTable('areas', function (table) {
      ...
      table
        ...
        .inTable('resorts')
        .onDelete('CASCADE')
      table.string('base_area', 50).notNullable();
      table.string('area_name', 50).notNullable();
    ...
    await knex.schema.createTable('runs', function (table) {
      ...
      table.boolean('is_terrain_park').defaultTo(false).notNullable();
      table.boolean('backcountry_access').defaultTo(false).notNullable();
      table.boolean('bootpack_req').defaultTo(false).notNullable();
      table.timestamp('date_created').defaultTo(knex.fn.now());
    });
